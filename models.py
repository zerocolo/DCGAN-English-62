...
class DCGAN(object):
    def __init__(self, sess, input_height=128, input_width=128, crop=True,
                 batch_size=64, sample_num=32, output_height=128, output_width=128,
                 y_dim=62, z_dim=100, gf_dim=64, df_dim=64,
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, int(batch_idxs)):
                ...
                if np.mod(counter, 500) == 0:
                    ...
                    save_images(samples, [8, 8], './samples/traint_arange_%s.png' % (idx))
    ...
    def load_data(self):
        ...
        imgs = np.zeros((62992, 28, 28, 3))
        labels = np.zeros((62992), dtype=np.int)
        label = 0
        i = 0

        classes = sorted(os.walk(self.data_dir).__next__()[1])

        for c in classes:
            c_dir = os.path.join(self.data_dir, c)
            walk = os.walk(c_dir).__next__()[2]

            for id, sample in enumerate(walk):
                img = cv2.imread(str(c_dir + '/' + sample))
        ...
        y_vec = np.zeros((len(labels), 62), dtype=np.float)
